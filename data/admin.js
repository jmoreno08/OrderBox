...
function renderProducts(){productsList.innerHTML=P.map(p=>`<div class="admin-row"><img class="mini-img" src="${p.image||'/img/placeholder.svg'}" onerror="this.src='/img/placeholder.svg'"><div><b>${p.name}</b><br><small>${p.category||'General'} - ${money(p.price)} - ${p.active?'Activo':'Inactivo'}</small></div><button onclick="editProduct(${p.id})">Editar</button><button class="danger-btn" onclick="removeProduct(${p.id})">Eliminar</button></div>`).join('');}
...
function renderHistory(){
  ...
  historyList.innerHTML=list.map(o=>`<div class="admin-row"><span class="logo">#${o.id}</span><div><b>${statusLabel(o.status)} - ${o.sourceType==='counter'?'Mostrador '+o.counterNumber:'Mesa '+o.table}</b><br><small>${o.items.map(i=>i.qty+'x '+i.name).join(', ')} - ${money(orderTotal(o))}</small></div><button class="danger-btn" onclick="deleteOrder(${o.id})">Eliminar</button></div>`).join('');
}
